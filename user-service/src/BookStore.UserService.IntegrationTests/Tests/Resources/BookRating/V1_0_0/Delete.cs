...
public class Delete : IDisposable
{
    ...
    [Fact]
    public async Task Delete_ValidRequest_BookRatingIsDeleted()
    {
        //Arrange
        await DatabaseTestHelper.Initialize(_databaseInits, _dbContext);

        var request = new DeleteRequest
            {RatingId = InitDataCreator.BookRatings.EugeneOneginTestUserRating.RatingId};
        //Act
        _bookRatingResource.Delete(bookRating => bookRating.RatingId = request.RatingId);
